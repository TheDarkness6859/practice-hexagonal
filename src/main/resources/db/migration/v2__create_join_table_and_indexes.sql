...
CREATE TABLE coders_categories (
                                   ...
                                   CONSTRAINT fk_cc_categoria FOREIGN KEY (categoria_id) REFERENCES categorias(id) ON DELETE CASCADE
);

-- index to improve the performance in supabase
CREATE INDEX idx_coders_clan ON coders(clan_id);
CREATE INDEX idx_coders_active ON coders(activo);
CREATE INDEX idx_clanes_names ON clanes(nombre);